...
TGGENCTL_CLI_SRC_DIR := $(shell pwd)/cli
TGGENCTL_CLI_NAME := tfgenctl-cli
...
cli-build:
	...
	@cd $(TGGENCTL_CLI_SRC_DIR) &&  $(GO_BUILD_SCRIPT) --binary $(TGGENCTL_CLI_NAME) --path ./main.go

## Run Go source code
.PHONY: go-run
cli-run:
	@echo "===========> Running binary of the tfgenctl-cli"
	@cd $(TGGENCTL_CLI_SRC_DIR) && ./$(TGGENCTL_CLI_NAME) $(ARGS)

.PHONY: go-run
cli-run-src:
	@echo "===========> Running source code"
	@$(GO) run $(TGGENCTL_CLI_SRC_DIR)/main.go $(ARGS)
